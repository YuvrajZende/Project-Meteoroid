...
import { getQueueAgentService } from './services/agents/queue-agent-service.js';
import { getTestAgentService } from './services/agents/test-agent-service.js';
...
async function bootstrap(): Promise<void> {
    try {
        ...
        const queueAgentService = getQueueAgentService();
        const queueStatus = queueAgentService.getStatus();
        logger.status('Queue Agent', queueStatus.initialized ? `Ready (${queueStatus.capabilities} caps)` : 'Not initialized', queueStatus.initialized || process.env.QUEUE_AGENT_ENABLED !== 'false');
