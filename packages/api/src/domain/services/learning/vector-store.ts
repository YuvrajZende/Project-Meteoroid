...
export class VectorStoreService {
    ...
    constructor(@unmanaged() config?: Partial<VectorStoreConfig>) {
        this.config = {
     ...
     */
    chunkContent(content: string, _filePath: string): Array<{
        content: string;
        startLine: number;
        endLine: number;
    }> {
    ...
    async getRelevantContext(
        ...
        const { maxChunks = 5, maxTokens = 2000, includeImports: _includeImports = true } = options;
