...
import { getSupabaseClient } from '../../database/database-client.js';
...
const ENCRYPTION_KEY = process.env.CREDENTIALS_ENCRYPTION_KEY || randomBytes(32).toString('hex');
const SCRYPT_SALT = process.env.SCRYPT_SALT || 'default-salt-change-in-production';
const ALGORITHM = 'aes-256-gcm';

// Derive a consistent 32-byte key from the configured key
 ...
 * Provides enterprise-grade encryption for sensitive credentials
 */
function encryptCredentials(credentials: Record<string, string>): string {
    try {
        const key = deriveKey(ENCRYPTION_KEY, SCRYPT_SALT);
        ...
        console.error('[ConnectionManager] Encryption failed:', error);
 ...
 * Handles both old base64 format (for migration) and new AES-256-GCM format
 */
function decryptCredentials(encrypted: string): Record<string, string> {
    try {
        ...
        // Fallback: handle old base64 format (for migration)
        const decoded = Buffer.from(encrypted, 'base64').toString('utf8');
        return JSON.parse(decoded);
    } catch (error) {
        console.error('[ConnectionManager] Decryption failed:', error);
