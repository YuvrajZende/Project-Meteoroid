...
import { checkSupabaseConnection } from '../infrastructure/database-client.js';
...
} from '../context/core-services.js';
import { getGenerationContext, type GenerationContextService, type GenerationContext } from '../context/generation-context.js';
...
// Learning & Analysis
import { getVectorStore, type VectorStoreService } from '../learning/vector-store.js';
import { getLearningService, type LearningService } from '../learning/learning-service.js';
import { getQualityAssessment, type QualityAssessmentService } from '../analysis/quality-assessment.js';
import { getArchitectureKnowledge, type ArchitectureKnowledgeService } from '../architecture/architecture-knowledge.js';
...
// Analysis (Phase 24)
import { getEntityExtractor, type EntityExtractorService } from '../analysis/entity-extractor.js';
import { getAIIntentAnalyzer, type AIIntentAnalyzer, type AIIntentAnalysis } from '../analysis/ai-intent-analyzer.js';
...
// Queue Agent Service (Person 2's Implementation - Local Wrapper)
import {
    getQueueAgentService,
    QueueAgentService,
    isQueueRelatedPrompt,
    extractQueueCapabilities,
} from '../agents/queue-agent-service.js';

// Test Agent Service (Person 2's Implementation - Local Wrapper)
import {
    getTestAgentService,
    TestAgentService,
    isTestRelatedPrompt,
    extractTestCapabilities,
} from '../agents/test-agent-service.js';

// AI Integration Service (Person 2's Implementation)
import {
...
export interface IntegratedOrchestratorConfig {
    ...
    /** Enable Queue Agent for background job generation */
    useQueueAgent: boolean;
    /** Enable Test Agent for automated test generation */
...
export interface OrchestrationStep {
    stepNumber: number;
    phase: 'init' | 'thinking' | 'analysis' | 'agent-selection' | 'execution' | 'code-generation' | 'multi-model' | 'enhanced-codegen' | 'cost-tracking' | 'learning' | 'quality' | 'architecture' | 'queue-generation' | 'finalize';
    agent?: string;
    message: string;
    timestamp: Date;
    duration?: number;
...
export class IntegratedOrchestrator {
    ...
    constructor(config?: Partial<IntegratedOrchestratorConfig>) {
        this.config = {
            ...
            useQueueAgent: config?.useQueueAgent ?? true, // Enable Queue Agent by default
            useTestAgent: config?.useTestAgent ?? true, // Enable Test Agent by default
        ...
        // Queue Agent Service (Person 2's Implementation)
        this.queueAgentService = getQueueAgentService();
        // Test Agent Service (Person 2's Implementation)
        this.testAgentService = getTestAgentService();
        // AI Integration Service (Person 2's Implementation)
        this.aiIntegrationService = getAIIntegrationService();
    }
    ...
    async initialize(): Promise<void> {
        ...
        const knownAgents = ['auth-agent', 'security-agent', 'api-agent', 'database-agent', 'monitoring-agent', 'queue-agent', 'test-agent'];
        for (const agent of knownAgents) {
            this.agentMonitor.registerAgent(agent);
        }

        // Initialize Queue Agent Service (Person 2's Implementation)
        if (this.config.useQueueAgent) {
            try {
                await this.queueAgentService.initialize();
                console.log('[ORCHESTRATOR] Queue Agent Service initialized');
            } catch (queueError) {
                console.warn('[ORCHESTRATOR] Queue Agent Service initialization failed:', queueError);
            }
        }

        // Initialize Test Agent Service (Person 2's Implementation)
        if (this.config.useTestAgent) {
            try {
                await this.testAgentService.initialize();
                console.log('[ORCHESTRATOR] Test Agent Service initialized');
            } catch (testError) {
                console.warn('[ORCHESTRATOR] Test Agent Service initialization failed:', testError);
            }
        }
        ...
        // Log Queue Agent Service status (Person 2's Implementation)
        if (this.config.useQueueAgent) {
            const queueStatus = this.queueAgentService.getStatus();
            console.log('╭──────────────────────────────────────────────────────────╮');
            console.log('│  ⚙️  QUEUE AGENT SERVICE (Person 2)                       │');
            console.log('├──────────────────────────────────────────────────────────┤');
            console.log(`│  Status              : ${queueStatus.initialized ? '✓ Ready' : '✗ Not Init'}                        │`);
            console.log(`│  Provider            : BullMQ + Redis                     │`);
            console.log(`│  Capabilities        : ${queueStatus.capabilities} total                          │`);
            console.log('╰──────────────────────────────────────────────────────────╯');
        }

        // Log Test Agent Service status (Person 2's Implementation)
        if (this.config.useTestAgent) {
            const testStatus = this.testAgentService.getStatus();
            console.log('╭──────────────────────────────────────────────────────────╮');
            console.log('│  🧪 TEST AGENT SERVICE (Person 2)                        │');
            console.log('├──────────────────────────────────────────────────────────┤');
            console.log(`│  Status              : ${testStatus.initialized ? '✓ Ready' : '✗ Not Init'}                        │`);
            console.log(`│  Provider            : Vitest/Jest/Playwright             │`);
            console.log(`│  Capabilities        : ${testStatus.capabilities} total                          │`);
            console.log('╰──────────────────────────────────────────────────────────╯');
        }
    ...
    async orchestrate(
        ...
        try {
            ...
            // ============================================
            // FAST PATH: QUEUE GENERATION (Person 2's Implementation)
            // ============================================
            if (this.config.useQueueAgent && isQueueRelatedPrompt(input.prompt)) {
                addStep('queue-generation', '⚙️ Detected queue-related request, using Queue Agent Service');

                try {
                    const queueCapabilities = extractQueueCapabilities(input.prompt);
                    addStep('queue-generation', `Detected capabilities: ${queueCapabilities.join(', ')}`, {
                        capabilities: queueCapabilities,
                    });

                    // Generate queue system
                    const queueResult = await this.queueAgentService.generate({
                        requirements: input.prompt,
                        generateWorkers: true,
                        generateScheduler: queueCapabilities.includes('job-scheduling'),
                        generateDLQ: true,
                        generateMonitoring: queueCapabilities.includes('queue-monitoring'),
                        generateFlows: queueCapabilities.includes('job-flows'),
                        generateRateLimiter: queueCapabilities.includes('rate-limiting'),
                    });

                    addStep('queue-generation', `Generated ${queueResult.files.length} queue system files`, {
                        files: queueResult.files.map(f => f.path),
                        dependencies: queueResult.dependencies,
                        envVariables: queueResult.envVariables,
                    });

                    // Convert queue files to generated code format
                    for (const file of queueResult.files) {
                        generatedCode.push({
                            subtask: `Queue: ${file.description || file.path}`,
                            code: file.content,
                            explanation: `Generated ${file.type} file: ${file.path}`,
                            agent: 'queue-agent',
                        });
                    }

                    // Write files if file writer is enabled
                    let fileWriteResult: WriteResult | undefined;
                    if (config.useFileWriter) {
                        const filesToWrite = queueResult.files.map(f => ({
                            path: f.path,
                            content: f.content,
                            type: 'code' as const,
                        }));
                        ...
                        fileWriteResult = await this.fileWriter.writeProject(
                            input.projectId,
                            filesToWrite,
                            { projectName: `queue-${input.taskId}` }
                        );

                        addStep('queue-generation', `Wrote ${fileWriteResult.filesWritten.length} queue files to disk`, {
                            filesWritten: fileWriteResult.filesWritten,
                            projectPath: fileWriteResult.projectPath,
                        });
                    }


                    const endTime = new Date();
                    addStep('finalize', '✅ Queue system generation complete');
                    ...
                    return {
                        ...
                        aiAnalysis: {
                            complexity: 'medium',
                            subtasks: queueResult.instructions,
                            suggestedAgents: ['queue-agent'],
                            estimatedSteps: queueResult.files.length,
                        },
                        steps,
                        agentsExecuted: ['queue-agent'],
                    ...
                    const errMsg = queueError instanceof Error ? queueError.message : 'Unknown error';
                    console.warn('[ORCHESTRATOR] Queue generation failed:', errMsg);
                    addStep('queue-generation', `Queue generation failed: ${errMsg} (continuing with normal pipeline)`);
...
`;
    ...
    getServices(): {
        ...
        return {
            ...
            // Queue Agent Service (Person 2's Implementation)
            queueAgentService: this.queueAgentService,
        };
    }
...
Start with the necessary imports.`;
    ...
    /**
     * Get the AI Integration Service for external use
     * Provides access to agent-specific AI generation capabilities
     */
    getAIIntegrationService(): AIIntegrationService {
        return this.aiIntegrationService;
    }

    /**
     * Get the Test Agent Service for external use
     * Provides access to test generation capabilities
     */
    getTestAgentService(): TestAgentService {
        return this.testAgentService;
    }

    /**
     * Get the Queue Agent Service for external use
     * Provides access to queue generation capabilities
     */
    getQueueAgentService(): QueueAgentService {
        return this.queueAgentService;
    }
}
