...
export class AIClient {
    ...
    constructor(config?: Partial<AIClientConfig>) {
        ...
        // Determine the model - only use MODEL_NAME if it matches the provider
        // Don't override provider-specific models with MODEL_NAME
        let finalModel = model;
        if (config?.model) {
            finalModel = config.model;
        } else if (process.env.MODEL_NAME) {
            // Only use MODEL_NAME for Z.AI/OpenAI providers, not for GROQ
            const isGroqEndpoint = baseUrl.includes('groq.com');
            const isOpenRouterEndpoint = baseUrl.includes('openrouter.ai');
            const isDeepSeekEndpoint = baseUrl.includes('deepseek.com');

            if (!isGroqEndpoint && !isOpenRouterEndpoint && !isDeepSeekEndpoint) {
                finalModel = process.env.MODEL_NAME;
            }
        }
        ...
        if (!this.config.apiKey) {
            ...
            console.warn('[AI-CLIENT] Set one of: GROQ_API_KEY, OPENAI_API_KEY, OPENROUTER_API_KEY, or DEEPSEEK_API_KEY');
        } else {
    ...
    private findAvailableApiKey(): string {
        // Priority order: GROQ (fast) > OPENAI > ZAI > OPENROUTER > DEEPSEEK > ANTHROPIC
        return process.env.GROQ_API_KEY  // Groq for fast inference
    ...
    private getDefaultConfig(apiKey: string): { baseUrl: string; model: string } {
        // If using Groq (RECOMMENDED for fast inference)
