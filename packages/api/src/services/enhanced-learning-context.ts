...
export class EnhancedLearningContextBuilder {
    ...
    private async getSimilarCode(
        ...
        try {
            ...
            // Use vector store to search by project similarity
            // Note: searchSimilar might not exist yet, using fallback
            const results: any[] = [];

            // TODO: Implement proper semantic search when vector store API is ready
            ...
            return results.map((result: any) => ({
                ...
                project: result.metadata?.project || 'unknown',
            }));
        } catch (error) {
    ...
    private async getStatistics(
        ...
        try {
            ...
            const successes = data.filter((row: any) =>
    ...
    /**
     * Helper: Extract language from context string
     */
    private extractLanguage(context: string | any): string {
        ...
        if (lowerContext.includes('typescript')) return 'typescript';
        if (lowerContext.includes('javascript')) return 'javascript';
        if (lowerContext.includes('python')) return 'python';
        if (lowerContext.includes('golang') || lowerContext.includes('go')) return 'go';
        if (lowerContext.includes('rust')) return 'rust';
        if (lowerContext.includes('java')) return 'java';
        if (lowerContext.includes('csharp') || lowerContext.includes('c#')) return 'csharp';

        return 'unknown';
    }

    /**
     * Helper: Extract framework from context string
     */
    private extractFramework(context: string | any): string {
        if (typeof context !== 'string') {
            try {
                context = JSON.stringify(context);
            } catch {
        ...
        if (lowerContext.includes('fastify')) return 'fastify';
        if (lowerContext.includes('express')) return 'express';
        if (lowerContext.includes('nestjs')) return 'nestjs';
        if (lowerContext.includes('fastapi')) return 'fastapi';
        if (lowerContext.includes('django')) return 'django';
        if (lowerContext.includes('flask')) return 'flask';
        if (lowerContext.includes('gin')) return 'gin';
        if (lowerContext.includes('spring')) return 'spring';
