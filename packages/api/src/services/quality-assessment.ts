...
export class QualityAssessmentService {
     ...
     */
    private calculateStructureScore(files: GeneratedFile[], language: string): number {
        let score = 50;
     ...
     */
    private calculateCodeQualityScore(files: GeneratedFile[], language: string): number {
        let totalScore = 0;
        let fileCount = 0;
    ...
    private buildRegenerationPrompt(
        issues: QualityIssue[],
        blueprint: ArchitectureBlueprint | undefined,
        language: string,
        framework: string
    ): string {
