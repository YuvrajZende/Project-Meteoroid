 ...
 * 
 * ARCH-001 REFACTORING: Now uses extracted services:
...
// Unified Generation Pipeline (Code Generation Fixes)
import { getUnifiedGenerationPipeline, type UnifiedGenerationPipeline } from '../validation/unified-generation-pipeline.js';
...
export class IntegratedOrchestrator {
    ...
    constructor(config?: Partial<IntegratedOrchestratorConfig>) {
        ...
        // ARCH-001: Initialize extracted services
    ...
    async initialize(): Promise<void> {
        ...
        console.log('├──────────────────────────────────────────────────────────┤');
        console.log('│  ✅ ARCH-001 REFACTORED SERVICES ACTIVE                   │');
        console.log('├──────────────────────────────────────────────────────────┤');
    ...
    async orchestrate(
        ...
        const addStep = (
            ...
            // REAL-TIME: Broadcast step to SSE for TUI
        ...
        try {
            ...
            // Set up context if enabled (ARCH-001: Using extracted service)
            ...
            if (intentAnalysis) {
                ...
                console.warn('[ORCHESTRATOR] Intent detection failed, continuing with defaults');
            }
...
`;
            ...
            if (config.useFileWriter && generatedCode.length > 0) {
                ...
                // ============================================
                // PHASE 5.0: UNIFIED GENERATION PIPELINE
                // ============================================
                // Apply all code generation fixes: deduplication, blueprint enforcement, 
                // import resolution, and final verification
                addStep('finalize', '🔧 Running Unified Generation Pipeline...');
                ...
                const pipelineResult = this.unifiedPipeline.execute(
                    ...
                    multiModelResult?.contextAnalysis?.architectureBlueprint || null,
                    projectLanguage,
                    input.context?.framework || 'fastify'
                );

                // Update filesToWrite with pipeline results
                filesToWrite = pipelineResult.files.map(f => ({
                    path: f.path,
                    content: f.content,
                    type: (f.type === 'doc' ? 'doc' : f.type === 'config' ? 'config' : 'code') as 'code' | 'config' | 'doc' | undefined,
                }));

                console.log(`[UNIFIED-PIPELINE] Stats: ${JSON.stringify(pipelineResult.stats)}`);

                if (!pipelineResult.verification.success) {
                    console.warn(`[UNIFIED-PIPELINE] Verification found ${pipelineResult.verification.errors.length} issues`);
                    for (const error of pipelineResult.verification.errors.slice(0, 3)) {
                        console.warn(`[UNIFIED-PIPELINE] ${error}`);
                    }
                }

                addStep('finalize', `🔧 Pipeline complete: ${pipelineResult.stats.duplicatesRemoved} duplicates removed, ${pipelineResult.stats.missingFilesGenerated} files generated`, {
                    duplicatesRemoved: pipelineResult.stats.duplicatesRemoved,
                    missingFilesGenerated: pipelineResult.stats.missingFilesGenerated,
                    importsResolved: pipelineResult.stats.importsResolved,
                    verificationPassed: pipelineResult.verification.success,
                });
